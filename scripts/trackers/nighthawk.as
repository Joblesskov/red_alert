...
class NighthawkRequest {
	...
	NighthawkRequest(int callId, int characterId, Vector3 targetPos, int factionId, float timer, int bursts) {
		m_callId = callId;
...
class NighthawkGunRun : Tracker {
  ...
  protected int m_hmg_rounds = 5; //number of hmg rounds the plane fires during a single burst
  ...
  NighthawkGunRun(Metagame@ metagame) {
    @m_metagame = @metagame;
  }
  ...
  protected void handleCallEvent(const XmlElement@ event) {
    ...
    // Check call key
    if (event.getStringAttribute("call_key") == "nighthawk.call") {
		...
		if (phase == "queue") {
			...
			NighthawkRequest@ thisCall = NighthawkRequest(callId, characterId, targetPos, factionId, m_arrivalTime + m_circleTime, m_burstCount);
			
			//the queue is necessary to handle multiple simultaneous AC130 requests
			NighthawkQueue.insertLast(thisCall);
		}

		//starting the timer and thus starting the fireworks :)
		if (phase == "launch") {
			//finding the launched call
			for (uint i = 0; i < NighthawkQueue.length() ; ++i){
				if (NighthawkQueue[i].m_callId == callId){
					//starting update function (if it's not running already)
					m_running = true;
					//starting the timer of the launched call
					NighthawkQueue[i].m_runGun = true;
					break;
				}
			}
		}
	...
	void update(float time) {
		...
		for (uint i = 0; i < NighthawkQueue.length() ; ++i){
			...
			if (NighthawkQueue[i].m_runGun == true){
				NighthawkQueue[i].m_timer -= time;
				
				//if it's time to fire, then scanning for targets
				if (NighthawkQueue[i].m_timer < (NighthawkQueue[i].m_bursts * m_cooldown)){
					string fireTarget = "empty";
					
					//scanning for vehicles in every odd numbered firing round (note that m_bursts counts backwards to 0)
					if (false && NighthawkQueue[i].m_bursts % 2 == 1){
						fireTarget = getVehicleTarget(NighthawkQueue[i], m_radius);
						
						//fireTarget = (NighthawkQueue[i].m_targetPos).toString(); //testing shots
						
						if (fireTarget != "empty"){
							// fireProjectiles(NighthawkQueue[i], fireTarget, "gunship_40mm.projectile", "ac130_autocannon_burst.wav", 5.0, 3, 0.3, 0, 1.4);
							// fireProjectiles(NighthawkQueue[i], fireTarget, "gunship_105mm.projectile", "ac130_cannon_shot.wav", 3.0, 1, 0.0, 4 * m_delayTime);
					...
					if (fireTarget == "empty") {
						fireTarget = getInfantryTarget(NighthawkQueue[i], m_radius);
						//fireTarget = (NighthawkQueue[i].m_targetPos).toString(); //testing shots
						
						//if a target is found, then firing
						if (fireTarget != "no_target"){
							fireProjectiles(NighthawkQueue[i], fireTarget, "nighthawk_bullet.projectile", "vblhatta.wav", 2.0, m_hmg_rounds, 0.08, 0, 2.0);
							// fireProjectiles(NighthawkQueue[i], fireTarget, "gunship_25mm.projectile", "ac130_hmg_burst.wav", 5.0, m_hmg_rounds, 0.032, 0);
							// fireProjectiles(NighthawkQueue[i], fireTarget, "gunship_40mm.projectile", "ac130_autocannon_burst.wav", 3.0, 3, 0.3, (m_hmg_rounds + 1) * m_delayTime, 1.4);
							//note that the 40mm's offset value accounts for the 25mm's fireProjectiles duration
						}
					}
					//reducing number of remaining burst
					NighthawkQueue[i].m_bursts--;
					//if none remaining, the removing the request from the queue
					if (NighthawkQueue[i].m_bursts == 0){
						NighthawkQueue.removeAt(i);
						--i;
					}
				}
			}
		...
		if (NighthawkQueue.length() == 0) m_running = false;
	}
	...
	string getInfantryTarget(NighthawkRequest@ NighthawkRequest, float range) {
		//extracting data
		int factionId = NighthawkRequest.m_factionId;
		Vector3 targetPos = NighthawkRequest.m_targetPos;
	...
	string getVehicleTarget(NighthawkRequest@ NighthawkRequest, float range) {
		//extracting data
		int factionId = NighthawkRequest.m_factionId;
		Vector3 targetPos = NighthawkRequest.m_targetPos;
  	...
  	void fireProjectiles(NighthawkRequest@ NighthawkRequest, string fireTarget, string instanceKey, string soundFile, float spread, uint number, float delay, float offset, float speed = 1.0) {
		//extracting data
		int characterId = NighthawkRequest.m_characterId;
		int factionId = NighthawkRequest.m_factionId;
		...
		for (uint i = 0; i < number; ++i){		
			...
			Vector3 direction = gunRunVector(NighthawkRequest.m_timer - i * (m_delayTime + delay) - offset - m_delayTime);
